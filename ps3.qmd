...
boxplot(pred_M, pred_F, names=c("M","F"))
c(mean(pred_M), mean(pred_F))
```
We can see that the predicted value of Tympanometric width measure of the left ear of women is greater than that of men.
                 ...
                 SELECT s.store_id, 
                        ...
                        ROUND(
                             (SUM(CASE WHEN active = 1 THEN 1 
                              ELSE 0 END) * 100.0 / COUNT(customer_id)),
                             2) AS percent_active
                FROM store AS s 
...
```
Using only SQL is much faster than manual R.
...
One single SQL query is faster than SQL then R operations.
...
```{r, align = c}
apt_first_digits <- benford(apt_num, 1)
apt_first_digits
```

The distribution of the leading digits of the apartment numbers appear to follow Benford's Law with the significant result from p-value of $8.675 \times 10^{-4}$. I think this can be used as real data.
